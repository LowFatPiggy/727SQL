/*
    ...
    * PL/SQL 구조
    ...
    - [예외처리부 (EXCEPTION SECTION)] 
        : EXCEPTION로 시작, 예외발생시 해결하기 위한 구문을 미리 기술해둘 수 있는 부분   
*/
...
-- 1_2) 레퍼런스 타입 변수 선언 및 초기화 (어떤테이블의 어떤컬럼의 데이터타입으로 참조)
--      [표현식] 변수명 테이블명.컬럼명%TYPE;
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE; -- EMPLOYEE테이블에 EMP_ID컬럼 데이터타입으로
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('보너스 : ' || NVL(E.BONUS, 0)); -- 함수 가능
...
-- 단, 보너스 없는 사원은 보너스율 출력 전 '보너스 없는 사원입니다' 출력
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
...
BEGIN
    ...
    IF BONUS = 0
        THEN DBMS_OUTPUT.PUT_LINE('보너스 없는 사원입니다');
    END IF;    
...
/*
    2) FOR LOOP문
    FOR 변수 IN [REVERSE] 초기값..최종값 -- 초기값에서 최종값까지 매번1씩 증가
    LOOP
        반복적으로 실행할 구문;
    END LOOP;
*/
...
/*
    ...
    EXCEPTION 
        ...
        WHEN 예외면2 THEN 예외처리구문2;
        ...
        WHEN OTHER THEN 예외처리구문N;
